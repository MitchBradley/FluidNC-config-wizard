...
- `boards/<id>.board.yaml` -- one file per board. Filename must match
  `board.id` (snake_case), e.g. `corgi.board.yaml`.
- `chips/<chip_id>.chip.yaml` -- lower-level MCU/chip facts (referenced by
  `board.mcu_chip`, e.g. `esp32`), not covered by this guide.
- `boards/corgi.board.yaml` -- the most complete real example
  in this repo; worth reading alongside this guide.
- `boards/index.json` -- a generated manifest of board ids, regenerated by
  `tools/gen_board_index.py`; drives the wizard's dropdown only (not
  required for loading/testing a board -- see "Loading your new board in
  the wizard before it's 'official'" below).
...
resource you expect shows up with a sensible name. You don't need the file
to live in `boards/` yet to do this -- see the next section.

## Loading your new board in the wizard before it's "official"

The wizard's board dropdown is populated from a generated manifest,
`boards/index.json` (see `tools/gen_board_index.py`) -- it's a static site
with no server-side directory listing and no `file://` access, so it can't
discover `boards/*.board.yaml` on its own; something has to hand it the
list. That manifest only drives the *dropdown*, though, and there are two
ways to load and test a board.yaml without it (useful mid-draft, before a
file even has a permanent home in `boards/`, or before you've bothered
regenerating the manifest):

1. **`index.html?board=<id>`** -- if the file already lives at
   `boards/<id>.board.yaml` in your local checkout, this URL param loads it
   directly, bypassing the dropdown/manifest entirely.
2. **The picker's "Custom (choose a local file)..." option** (last item in
   the dropdown) -- opens a file-picker dialog and loads whatever
   `board.yaml` you select from anywhere on disk, with no requirement that
   it live in this repo's `boards/` folder at all. This is the fastest path
   for an AI assistant's draft that hasn't been saved into `boards/` yet, or
   for comparing two candidate drafts side by side.

Only once you're happy with the file and it's saved as
`boards/<id>.board.yaml` should you regenerate the manifest so it shows up
in the dropdown for everyone else:

```
python3 tools/gen_board_index.py
```

This rewrites `boards/index.json` from whatever `*.board.yaml` files
currently exist in `boards/`. Commit the updated `boards/index.json`
alongside your new/changed board file(s) -- a PR that adds a board.yaml
without this update will still work (loadable via `?board=<id>` or the
picker's manifest-independent fallback), it just won't appear in the
dropdown until someone regenerates it.
